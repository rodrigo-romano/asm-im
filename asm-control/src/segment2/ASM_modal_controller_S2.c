/*
 * File: ASM_modal_controller_S2.c
 *
...
#include "ASM_modal_controller_S2.h"
#include "ASM_modal_controller_S2_private.h"

/* Block states (default storage) */
DW_ASM_modal_controller_S2_T ASM_modal_controller_S2_DW;

/* External inputs (root inport signals with default storage) */
ExtU_ASM_modal_controller_S2_T ASM_modal_controller_S2_U;

/* External outputs (root outports fed by signals with default storage) */
ExtY_ASM_modal_controller_S2_T ASM_modal_controller_S2_Y;

/* Real-time model */
RT_MODEL_ASM_modal_controller_S2_T ASM_modal_controller_S2_M_;
RT_MODEL_ASM_modal_controller_S2_T *const ASM_modal_controller_S2_M =
  &ASM_modal_controller_S2_M_;

/* Model step function */
void ASM_modal_controller_S2_step(void)
{
  ...
  for (i = 0; i < 66; i++) {
     ...
     *  Inport: '<Root>/ASM_cmd'
     */
    rtb_deltacmd[i] = ASM_modal_controller_S2_U.ASM_cmd[i] -
      ASM_modal_controller_S2_DW.Delay_DSTATE[i];

    /* Delay: '<S3>/Delay' */
    rtb_oldcmd[i] = ASM_modal_controller_S2_DW.Delay_DSTATE[i];
  }

  /* DiscretePulseGenerator: '<S3>/Pulse Generator' */
  rtb_PulseGenerator = ((ASM_modal_controller_S2_DW.clockTickCounter < 1) &&
                        (ASM_modal_controller_S2_DW.clockTickCounter >= 0));
  if (ASM_modal_controller_S2_DW.clockTickCounter >= 1) {
    ASM_modal_controller_S2_DW.clockTickCounter = 0;
  } else {
    ASM_modal_controller_S2_DW.clockTickCounter++;
  }
  ...
  if ((rtb_PulseGenerator > 0) &&
      (ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_PrevResetState <= 0)) {
    ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_DSTATE = 0.0;
  }
  ...
  rtb_Polynomial = ((6.1843584E+18 *
                     ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_DSTATE +
  ...
  for (i = 0; i < 5; i++) {
    rtb_Polynomial = rtb_Polynomial *
      ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_DSTATE +
      ASM_modal_controller_S2_ConstP.Polynomial_Coefs[i + 1];
  }
  ...
  for (i = 0; i < 66; i++) {
    ...
    if (ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_DSTATE <=
  ...
  for (i = 0; i < 66; i++) {
     ...
     *  Sum: '<S3>/Add'
     */
    rtb_Polynomial = (rtb_deltacmd_j - ASM_modal_controller_S2_U.ASM_FB[i]) -
      (-ASM_modal_controller_S2_DW.ASMPIcontroller_states[i]);
    ASMPIcontroller_tmp[i] = rtb_Polynomial;
    rtb_Polynomial *= 70031.657886510584;
    rtb_Product1[i] = -69968.342113489416 *
      ASM_modal_controller_S2_DW.ASMPIcontroller_states[i] + rtb_Polynomial;
  ...
  for (i = 0; i < 66; i++) {
     ...
     *  Polyval: '<S3>/Polynomial2'
     */
    rtb_Polynomial = ASM_modal_controller_S2_U.ASM_FB[i] - 0.22818630846850391 *
      ASM_modal_controller_S2_DW.Numericaldifferentiation_states[i];
    ...
    for (i_0 = 0; i_0 < 66; i_0++) {
      rtb_Product2_g += ASM_modal_controller_S2_ConstP.Ks_Gain[tmp + i] *
        rtb_deltacmd[i_0];
      tmp += 66;
    }

    ASM_modal_controller_S2_Y.ASM_modalF[i] = ((rtb_oldcmd[i] + rtb_Kb[i]) +
     ...
     *  Polyval: '<S3>/Polynomial2'
     */
    ASM_modal_controller_S2_Y.Mag_modal_vel[i] = rtb_Polynomial;
     ...
     */
    ASM_modal_controller_S2_DW.Delay_DSTATE[i] =
      ASM_modal_controller_S2_DW.Delay_DSTATE[i + 66];
  }

  /* Update for DiscreteIntegrator: '<S3>/Discrete-Time Integrator' */
  ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_DSTATE += 0.000125;
  if (rtb_PulseGenerator > 0) {
    ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_PrevResetState = 1;
  } else {
    ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_PrevResetState = 0;
  }
   ...
   *  Inport: '<Root>/ASM_cmd'
   */
  memcpy(&ASM_modal_controller_S2_DW.Delay_DSTATE[66],
         &ASM_modal_controller_S2_U.ASM_cmd[0], 66U * sizeof(real_T));

  /* Update for DiscreteTransferFcn: '<S1>/ASM PI controller' incorporates:
   *  Delay: '<S3>/Delay'
   */
  memcpy(&ASM_modal_controller_S2_DW.ASMPIcontroller_states[0],
         &ASMPIcontroller_tmp[0], 66U * sizeof(real_T));

  /* Update for DiscreteTransferFcn: '<S1>/Numerical differentiation' incorporates:
   *  Delay: '<S3>/Delay'
   */
  memcpy(&ASM_modal_controller_S2_DW.Numericaldifferentiation_states[0],
         &rtb_Product2[0], 66U * sizeof(real_T));
}

/* Model initialize function */
void ASM_modal_controller_S2_initialize(void)
{
  /* Registration code */

  /* initialize error status */
  rtmSetErrorStatus(ASM_modal_controller_S2_M, (NULL));

  /* states (dwork) */
  (void) memset((void *)&ASM_modal_controller_S2_DW, 0,
                sizeof(DW_ASM_modal_controller_S2_T));

  /* external inputs */
  (void)memset(&ASM_modal_controller_S2_U, 0, sizeof
               (ExtU_ASM_modal_controller_S2_T));

  /* external outputs */
  (void) memset((void *)&ASM_modal_controller_S2_Y, 0,
                sizeof(ExtY_ASM_modal_controller_S2_T));

  /* InitializeConditions for DiscretePulseGenerator: '<S3>/Pulse Generator' */
  ASM_modal_controller_S2_DW.clockTickCounter = 0;

  /* InitializeConditions for DiscreteIntegrator: '<S3>/Discrete-Time Integrator' */
  ASM_modal_controller_S2_DW.DiscreteTimeIntegrator_PrevResetState = 2;
}

/* Model terminate function */
void ASM_modal_controller_S2_terminate(void)
{
