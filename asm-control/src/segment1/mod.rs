...
build_controller!(ASM_modal_controller_S1,
...
impl<'a> IOTags for Controller<'a> {
    fn outputs_tags(&self) -> Vec<Tags> {
        ios!(ASMCmd, M2S1FSRBModalD)
    }
    fn inputs_tags(&self) -> Vec<Tags> {
        ios!(M2S1FSCPModalF, M2S1FSRBModalF)
    }
}
...
impl<'a> Dos for Controller<'a> {
    ...
    fn inputs(&mut self, data: Option<Vec<IO<Self::Input>>>) -> Result<&mut Self, DOSIOSError> {
        if let Some(data) = data {
            match (&data[ios!(M2S1Cmd)], &data[ios!(M2S1FSRBModalD)]) {
                (
                    IO::M2S1Cmd {
                        data: Some(asm_cmd),
                    },
                ...
                _ => Err(DOSIOSError::Inputs(
                    "Missing IO::ASMCmd and IO::M2S1FSRBModalD as inputs to ASM controller".into(),
                )),
            }
        } else {
...
mod tests {
    ...
    fn cmd_piston_test() {
        ...
        for k in 0..10 {
            ...
            asm_cmd[0] = 1e-6;
            let u = ios!(M2S1FSRBModalD(vec![0f64; 66]), ASMCmd(asm_cmd));
            let y = asm_ctrl.in_step_out(Some(u)).unwrap().unwrap();
            println!(
                " {}: {:#?}",
                k,
                Option::<Vec<f64>>::from(&y[ios!(M2S1FSCPModalF)]).unwrap()[0]
            );
        }
    }
    ...
    fn fdb_piston_test() {
        ...
        for k in 0..10 {
            let mut asm_cmd = vec![0f64; 66];
            asm_cmd[0] = 1e-6;
            let u = ios!(M2S1FSRBModalD(asm_cmd), ASMCmd(vec![0f64; 66]));
            let y = asm_ctrl.in_step_out(Some(u)).unwrap().unwrap();
            println!(
                " {}: {:#?}",
                k,
