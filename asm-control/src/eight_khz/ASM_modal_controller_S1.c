...
void ASM_modal_controller_S1_step(void)
{
  ...
  if (ASM_modal_controller_S1_DW.clockTickCounter >= 1) {
    ASM_modal_controller_S1_DW.clockTickCounter = 0;
  } else {
  ...
  for (i = 0; i < 66; i++) {
     ...
     *  Polyval: '<S3>/Polynomial2'
     */
    rtb_Polynomial = ASM_modal_controller_S1_U.ASM_FB[i] - 0.22818630846850391 *
    ...
    for (i_0 = 0; i_0 < 66; i_0++) {
      rtb_Product2_g += ASM_modal_controller_S1_ConstP.Ks_Gain[tmp + i] *
        rtb_deltacmd[i_0];
      tmp += 66;
    }
     ...
     */
    ASM_modal_controller_S1_DW.Delay_DSTATE[i] =
      ASM_modal_controller_S1_DW.Delay_DSTATE[i + 66];
  }

  /* Update for DiscreteIntegrator: '<S3>/Discrete-Time Integrator' */
  ASM_modal_controller_S1_DW.DiscreteTimeIntegrator_DSTATE += 0.000125;
  if (rtb_PulseGenerator > 0) {
   ...
   *  Inport: '<Root>/ASM_cmd'
   */
  memcpy(&ASM_modal_controller_S1_DW.Delay_DSTATE[66],
