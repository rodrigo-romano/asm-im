...
import_simulink!(ASM_modal_controller_S2, U : (ASM_cmd,66,ASM_FB,66), Y : (ASM_modalF,66,Mag_modal_vel,66));
build_inputs!(ASMCmd, 66, ASMFB, 66);
build_outputs!(ASMModalF, 66, MagModalVel, 66);
build_controller!(ASM_modal_controller_S2,
...
impl<'a> IOTags for Controller<'a> {
    fn outputs_tags(&self) -> Vec<Tags> {
        ios!(ASMCmd, M2S2FSRBModalD)
    }
    fn inputs_tags(&self) -> Vec<Tags> {
        ios!(M2S2FSCPModalF, M2S2FSRBModalF)
    }
}
...
impl<'a> Dos for Controller<'a> {
    ...
    fn inputs(&mut self, data: Option<Vec<IO<Self::Input>>>) -> Result<&mut Self, DOSIOSError> {
        if let Some(data) = data {
            match (&data[ios!(M2S2Cmd)], &data[ios!(M2S2FSRBModalD)]) {
                (
                    IO::M2S2Cmd {
                        data: Some(asm_cmd),
                    },
                ...
                _ => Err(DOSIOSError::Inputs(
                    "Missing IO::ASMCmd and IO::M2S2FSRBModalD as inputs to ASM controller".into(),
                )),
            }
        } else {
...
mod tests {
    ...
    fn cmd_piston_test() {
        ...
        for k in 0..10 {
            ...
            asm_cmd[0] = 1e-6;
            let u = ios!(M2S2FSRBModalD(vec![0f64; 66]), ASMCmd(asm_cmd));
            let y = asm_ctrl.in_step_out(Some(u)).unwrap().unwrap();
            println!(
                " {}: {:#?}",
                k,
                Option::<Vec<f64>>::from(&y[ios!(M2S2FSCPModalF)]).unwrap()[0]
            );
        }
    }
    ...
    fn fdb_piston_test() {
        ...
        for k in 0..10 {
            let mut asm_cmd = vec![0f64; 66];
            asm_cmd[0] = 1e-6;
            let u = ios!(M2S2FSRBModalD(asm_cmd), ASMCmd(vec![0f64; 66]));
            let y = asm_ctrl.in_step_out(Some(u)).unwrap().unwrap();
            println!(
                " {}: {:#?}",
                k,
